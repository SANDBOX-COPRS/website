...
var urlStart = ""
if (isLocal == true) {
    urlStart = "http://127.0.0.1" // Local 
} else {
    urlStart = "" // "https://dev.reprocessing-preparation.ml" // Dev deployement
}
...
/**
 * DEUBG : Display console among debug value
 * @param {*} str 
 * @param {*} obj 
 */
...
var getUrlDataParams = function(url) {
    ...
    var mission = $("#txtMission_id")[0].value
    var unit = $("#txtUnit_id")[0].value
    var productType = $("#txtProductType_id")[0].value
...
/**
 * Get list of aux files (promise function)
 * @param {*} filter 
 * @returns 
 */
...
var getReprocessingAuxTypes = function(filter) {
    return new Promise((successCallback, failureCallback) => {
        var xhr = new XMLHttpRequest()
        xhr.open("GET", getUrlParams(urls.reprocessingConfigBaseline.auxTypes,filter), true)
        var bearer="Bearer "+token
 ...
 * @returns 
 */
var getReprocessingData = function(filter) {
    return new Promise((successCallback, failureCallback) => {
        var xhr = new XMLHttpRequest()
